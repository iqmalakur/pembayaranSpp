    ...
    <tbody id="container-cari">
        <?php foreach ($petugas as $item) : ?>
            <tr>
                ...
                <td><?= $item->level; ?></td>
                <td>
                    <a href="/petugas/edit/<?= $item->username; ?>" class="btn btn-primary <?= $item->username !== $user->username && $user->username !== 'admin' && $item->level === 'admin' ? 'disabled' : ''; ?>" title="Ubah"><i class="bi bi-pencil"></i></a>
                    <form action="/petugas/delete/<?= $item->username; ?>" method="POST" class="d-inline">
                        <?= csrf_field(); ?>
                        <input type="hidden" name="_method" value="DELETE">
                        <span class="delete btn btn-danger <?= ($item->username == 'admin' && $user->username == 'admin') || ($item->level === 'admin' && $user->username !== 'admin') ? 'disabled' : ''; ?>" data-item="<?= $item->nama_petugas; ?>" title="Hapus"><i class="bi bi-trash"></i></span>
                    </form>
                </td>
            </tr>
        <?php endforeach ?>
